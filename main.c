...
void processUser(UserShoppingList* shoppingList){
 ...
 int orderThreadCreated = pthread_create(&orderThread, NULL, calculateStoreBaskettValue, (void*)shoppingList);
 if (orderThreadCreated == 0) {
   char orderThreadLogMsg[MAX_PATH_LEN];
   snprintf(orderThreadLogMsg, sizeof(orderThreadLogMsg),
            "pid %d create thread for orders tid:%ld",
            getpid(), orderThread);
   writeToLogFile("Dataset", shoppingList->userID, 0, orderThreadLogMsg);
 }




 // Create and log score thread
 pthread_t scoreThread;
 int scoreThreadCreated = pthread_create(&scoreThread, NULL, rateProducts, (void*)shoppingList);
 if (scoreThreadCreated == 0) {
   char scoreThreadLogMsg[MAX_PATH_LEN];
   snprintf(scoreThreadLogMsg, sizeof(scoreThreadLogMsg),
            "pid %d create thread for score tid:%ld",
            getpid(), scoreThread);
   writeToLogFile("Dataset", shoppingList->userID, 0, scoreThreadLogMsg);
 }




 // Create and log final thread
 /*pthread_t finalThread;
 int finalThreadCreated = pthread_create(&finalThread, NULL, finalThreadFunction, (void*)shoppingList);
 if (finalThreadCreated == 0) {
   char finalThreadLogMsg[MAX_PATH_LEN];
   snprintf(finalThreadLogMsg, sizeof(finalThreadLogMsg),
            "pid %d create thread for final tid:%ld",
            getpid(), finalThread);
   writeToLogFile("Dataset", shoppingList->userID, 0, finalThreadLogMsg);
 }*/


 // First call processStores()
